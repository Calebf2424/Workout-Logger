...
def get_routine_by_id(routine_id):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("SELECT id, name FROM planned_routines WHERE id = %s", (routine_id,))
            return c.fetchone()
...
def update_routine_name(routine_id, new_name):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("""
...
def delete_routine_set_by_id(set_id):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("DELETE FROM routine_sets WHERE id = %s", (set_id,))
